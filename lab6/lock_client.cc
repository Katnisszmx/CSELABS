// RPC stubs for clients to talk to lock_server

#include "lock_client.h"
#include "rpc.h"
#include <unistd.h>
#include <arpa/inet.h>

#include <sstream>
#include <iostream>
#include <stdio.h>

lock_client::lock_client(std::string dst)
{
    sockaddr_in dstsock;
    make_sockaddr(dst.c_str(), &dstsock);
    cl = new rpcc(dstsock);
    if (cl->bind() < 0) {
        printf("lock_client: call bind\n");
    }
}

lock_protocol::status lock_client::stat(lock_protocol::lockid_t lid)
{
    int r;
    lock_protocol::status ret = cl->call(lock_protocol::stat, cl->id(), lid, r);
    VERIFY (ret == lock_protocol::OK);
    return r;
}

lock_protocol::status lock_client::acquire(lock_protocol::lockid_t lid)
{
    int r;
    lock_protocol::status ret = cl->call(lock_protocol::acquire, cl->id(), lid, r);
    VERIFY (ret == lock_protocol::OK);
    pthread_t tid;
    args arg;
    arg.lcp = this;
    arg.lid = lid;
    pthread_create(&tid, NULL, heartbeat, &arg);
    ltmap[lid] = tid;
    return ret;
}

lock_protocol::status lock_client::release(lock_protocol::lockid_t lid)
{
    int r;
    lock_protocol::status ret = cl->call(lock_protocol::release, cl->id(), lid, r);
    VERIFY (ret == lock_protocol::OK);
    pthread_cancel(ltmap[lid]);
    ltmap.erase(lid);
    return ret;
}

void* lock_client::heartbeat(void* arg){
    int r;
    lock_client* lcp = ((args*)arg)->lcp;
    lock_protocol::lockid_t lid = ((args*)arg)->lid;
    while(true){
        sleep(1);
        lcp->cl->call(lock_protocol::heartbeat, lcp->cl->id(), lid, r);
    }
    pthread_exit(NULL);
}
